...
async def get_inventory(db: Session = Depends(get_db)):
    """Return all inventory items (global + user-specific)."""
    items = db.query(InventoryItem).order_by(InventoryItem.sku).all()
    return {
        "inventory": [
            {
                "id": i.id,
...
async def upsert_inventory_item(
    ...
    if existing:
        ...
        db.commit()
        _log_audit(db, user.id, "update_inventory", "inventory", existing.id, item)
    ...
    db.commit()
    db.refresh(new_item)
    _log_audit(db, user.id, "create_inventory", "inventory", new_item.id, item)
